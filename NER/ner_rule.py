...
def Bi_match(query, dic):
    ...
    #返回分词数较少者
    if len(forward_match) > len(backward_match):
        return backward_match
    elif len(forward_match) < len(backward_match):
...
if __name__ == '__main__':
    word_dic = {'北京大学': 'org', '算法工程师': 'post', '工程师': 'post', '岗': 'else'}
    print(forward_match('北京大学生前来应聘算法工程师岗', word_dic))
    print(backward_match('北京大学生前来应聘算法工程师岗', word_dic))
